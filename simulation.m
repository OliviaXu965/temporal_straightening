...
ExpParam.avgLocCurv = 60;
ExpParam.numTrials  = 1000; 
ExpParam.numDim     = 3; %dimensionality of the perceptual space
...
% vector
min_length = 1; max_length = 3; %longer length = better performance
...
%% generate n-d isotropic gaussians centered on each frame (visualization for 2d)
...
% for i = 1:ExpParam.numFrames
%     gaussians{i} = mvnpdf(grid_vals, mus(:, i)', eye(numDim) * sigma);
...
%% save data
perceptual_traj_sim_data = {ExpParam, Data};
save('perceptual_traj_sim_data.mat', 'perceptual_traj_sim_data');
